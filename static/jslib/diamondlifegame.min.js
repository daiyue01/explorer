...
function CreateLifeGameInitialSVG(lifegenehex, wide, bkclr, notborder) {
    ...
    // ret SVG
    if(partcolorful) {
        lifecolor = 'partcolorful'
    }
    if(fullcolorful) {
        lifecolor = 'fullcolorful'
    }
    var vnw = notborder ? 180 : 200
    return ['<svg class="dvhip9" version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+vnw+' '+vnw+'" width="'+wide+'" height="'+wide+'"><g>'+ptdraws.join('')+'</g></svg>',
    // config parkage
    {
        gene: gene,
        points: points,
        background: backcolor, // none
        lifecolor: lifecolor, // fullcolorful, partcolorful
...
function StartPlayLifeGame(wrapelm, lgcnf, lifewidth, steptime, backclr) {
    ...
    , backcolor = lgcnf.background || backclr
    ;
    ...
    var lifecolorfn = function (i, k){
        var cls = ['red', 'orange', 'yellow', 'green', 'cyan', 'blue', 'purple']
        if(lgcnf.lifecolor == 'fullcolorful'){
